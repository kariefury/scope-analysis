...
packet_indices = [(3,129),(3,150),(3,123)]
	...
	for each in data_chunk:
		...
		try:
			if len(s) == 3:
				if (i == 0 ):
					x_vals[i].append(j)
					if (float(s[1]) > 0.2) and (firstThreshold[i] == 0):
						firstThreshold[i] = j
				else:
					if( float(s[1]) > 0.2) and (firstThreshold[i] == 0):
						firstThreshold[i] = j
						x_vals[i].append(j)
					elif (firstThreshold[i] != 0):
						if (firstThreshold[i] == firstThreshold[0]):
							x_vals[i].append(j)
						else:

							offset = ( firstThreshold[0] - firstThreshold[i] )
							x_vals[i].append(j+offset)
					else:
						x_vals[i].append(j)
				j += 1
